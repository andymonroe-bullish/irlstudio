...
const getInitialRevenueStreams = (): RevenueStream[] => [
  { id: "1", name: "Core Mastermind Seat", price: 1600, projectedVolume: 15 },
  { id: "2", name: "Premium Mastermind Pass", price: 2200, projectedVolume: 10 },
  { id: "3", name: "Exclusive Partner Sponsorship", price: 4000, projectedVolume: 1 },
  { id: "4", name: "Post-Mastermind Group Coaching", price: 750, projectedVolume: 5 },
];

const ProjectionsManager = ({ totalBudget }: ProjectionsManagerProps) => {
  const [revenueStreams, setRevenueStreams] = useState<RevenueStream[]>(getInitialRevenueStreams);

  const totalRevenue = revenueStreams.reduce(
    (sum, stream) => sum + stream.price * stream.projectedVolume,
    0
  );
  const projectedProfit = totalRevenue - totalBudget;
  const roi = totalBudget > 0 ? totalRevenue / totalBudget : 0;

  const handleUpdateStream = (id: string, field: keyof RevenueStream, value: string | number) => {
                  ...
                  <tr key={stream.id} className="group">
                    <td className="py-4">
                      <Input
                        value={stream.name}
                        onChange={(e) =>
                          handleUpdateStream(stream.id, "name", e.target.value)
                        }
                    ...
                    <td className="py-4 text-right font-medium text-primary">
                      {formatCurrency(stream.price * stream.projectedVolume)}
                    </td>
                    <td className="py-4">
                      <button
