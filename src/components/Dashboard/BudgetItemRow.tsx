    ...
    <Draggable draggableId={item.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={cn(
            "group border-b border-border/50 transition-all",
            snapshot.isDragging && "shadow-lg bg-card border border-primary/20 rounded-lg"
          )}
        >
          {/* Desktop View */}
          <div className="hidden md:grid grid-cols-[1fr,120px,100px,80px,100px,40px] gap-4 items-center py-4 px-4 hover:bg-secondary/30">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 rounded hover:bg-destructive/10 hover:text-destructive transition-all"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
                {isExpanded ? (
              ...
              <div className="px-4 pb-4 space-y-3 bg-muted/20">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      type="number"
                      value={item.estimatedCost}
                      onChange={(e) => onUpdate(item.id, { estimatedCost: Number(e.target.value) })}
                      className="h-9"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={item.actualCost}
                      onChange={(e) => onUpdate(item.id, { actualCost: Number(e.target.value) })}
                      className="h-9"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="bg-card border-border w-full">
                      {BUDGET_CATEGORIES.map((cat) => (
                        <DropdownMenuItem
                          key={cat.id}
                          onClick={() => onUpdate(item.id, { category: cat.id })}
